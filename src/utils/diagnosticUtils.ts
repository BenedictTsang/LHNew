...
export async function checkPicaNotionConnection(): Promise<DiagnosticCheck> {
  const check: DiagnosticCheck = {
    id: 'pica-notion-connection',
    name: 'PicaOS/Notion Database',
    status: 'running'
  };
  ...
  try {
    ...
    if (response.ok) {
      ...
      return {
        ...
        details: `Found ${activityCount} published ${activityCount === 1 ? 'activity' : 'activities'} in Notion database`
      };
    }
    ...
    if (response.status === 401) {
      const isInvalidToken = errorDetails.includes('Invalid token') || errorDetails.includes('unauthorized');
      return {
        ...check,
        status: 'failed',
        message: isInvalidToken
          ? 'Notion API key is invalid or expired'
          : 'Authentication failed with Notion API',
        details: `Status 401: ${errorDetails || errorMessage}. FIX: Go to notion.so/my-integrations, create a new integration or regenerate the token, then update NOTION_API_KEY in Supabase Edge Function secrets.`
      };
    }
    ...
    if (response.status === 403) {
      return {
        ...check,
        status: 'failed',
        message: 'Notion integration lacks permission to access database',
        details: `Status 403: ${errorDetails || errorMessage}. FIX: Open your Notion database, click "..." menu > "Add connections" > select your integration. The database must be explicitly shared with the integration.`
      };
    }

    if (response.status === 404) {
      const isDatabaseNotFound = errorDetails.includes('Could not find database') || errorDetails.includes('object_not_found');
      if (isDatabaseNotFound) {
        return {
          ...check,
          status: 'failed',
          message: 'Notion database not found or not shared with integration',
          details: `Status 404: Database ID may be incorrect, OR the database exists but is not shared with the Notion integration. FIX: 1) Verify the database ID in the edge function matches your Notion database URL. 2) Open the database in Notion > click "..." > "Add connections" > select your integration.`
        };
      }
      return {
        ...check,
        ...
        details: `Status 404: ${errorDetails || errorMessage}`
      };
    }

    if (response.status === 400) {
      const isValidationError = errorDetails.includes('validation_error') || errorDetails.includes('invalid');
      return {
        ...check,
        status: 'failed',
        message: isValidationError
          ? 'Invalid request to Notion API'
          : 'Bad request',
        details: `Status 400: ${errorDetails || errorMessage}. This may indicate the database schema doesn't match expected properties (Name, Status, Difficulty, Questions).`
      };
    }
    ...
    if (response.status >= 500) {
      return {
        ...check,
        status: 'failed',
        message: 'Notion API server error',
        details: `Status ${response.status}: Notion's servers may be experiencing issues. Check status.notion.so for outages.`
      };
    }
    ...
    if (err instanceof Error && err.name === 'AbortError') {
      return {
        ...
        details: 'Request to PicaOS/Notion took longer than 10 seconds. Check network connectivity and Supabase Edge Function status.'
      };
    }
    ...
    if (err instanceof TypeError && err.message.includes('fetch')) {
      return {
        ...check,
        status: 'failed',
        message: 'Network error - cannot reach edge function',
        details: 'Failed to connect to Supabase. Check your internet connection and verify VITE_SUPABASE_URL is correct.'
      };
    }
...
export async function checkPicaEdgeFunctionDeployed(): Promise<DiagnosticCheck> {
  const check: DiagnosticCheck = {
    id: 'pica-edge-deployed',
    name: 'PicaOS Edge Function',
    status: 'running'
  };
  ...
  try {
    ...
    if (response.ok || response.status === 200 || response.status === 204) {
      return {
        ...
        details: 'CORS preflight check passed'
      };
    }
    ...
    if (response.status === 404) {
      return {
        ...
        details: 'The pica-notion edge function does not exist. FIX: Deploy the function using Supabase CLI or dashboard.'
      };
    }
    ...
    return {
      ...
      details: 'Function exists but returned unexpected status'
    };
  } catch (err) {
    if (err instanceof Error && err.name === 'AbortError') {
      return {
        ...
        details: 'Function did not respond within 5 seconds'
      };
    }
...
export function getPageChecks(
  ...
  const configs: Record<string, PageCheckConfig> = {
    ...
    'learning-hub': {
      checks: [
        ...baseChecks,
        () => checkPicaEdgeFunctionDeployed(),
        () => checkPicaNotionConnection(),
      ]
    },
  };
